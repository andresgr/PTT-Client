        ...
        implements SipListener
{
    ...
    /**
     * The registrar Address Domain
     */
    private String registrarDomain = null;
     ...
     * registering.
     */
    public static final int DEFAULT_REGISTRATION_EXPIRATION = 30000;
    ...
    ClientTransaction regTrans = null;
     ...
     public static long tiempoInicioRegistro = 0;
     public static long tiempoInicioDesRegistro  = 0;
    ...
    public SipRegistrarConnection(InetAddress registrarAddress,
        ...
        throws ParseException
    {
        this(registrarAddress,registrarPort,registrationTransport,expirationTimeout, sipProviderCallback);
        ...
        throws OperationFailedException
    {
        ...
        try {
			//String s = sipProvider.getRandomHex(43).concat(".");
            //logger.debug("Longitud de call-id: " + s.length());
            //callIdHeader.setCallId(s);
            callIdHeader.setCallId(sipProvider.getRandomString(43).concat("."));
        ...
        try
        {
            ...
            //logger.info("Adding IMS headers...");
            if (sipProvider.hasRouteHeader(request.getMethod())) {
                logger.debug("Anadiendo cabecera ROUTE...");
                request.addHeader(sipProvider.getRouteHeader(request.getMethod()));
            }

            /*request.addHeader(sipProvider.getPrivacyHeader("None"));
            request.addHeader(sipProvider.getPAccessNetworkInfoHeader(""));
            request.addHeader(sipProvider.getPPreferredIdentityHeader());*/
            request.addHeader(sipProvider.getIMS_OSGi_ClientAgentHeader());
            request.addHeader(sipProvider.getAllowHeader());
        ...
        UserAgentHeader userAgentHeader
            = sipProvider.getIMS_OSGi_ClientAgentHeader();
        ...
        ContactHeader contactHeader
            = sipProvider.getContactHeader(
                registrarAddress, getRegistrarListeningPoint(),true);
        ...
        /*try {
			contactHeader.setParameter("rinstance", sipProvider.getRandomHex(16));
		} catch (ParseException e) {
			e.printStackTrace();
		}*/
        ...
        /*try {
            SupportedHeader sh = sipProvider.getHeaderFactory().createSupportedHeader("path");
            request.addHeader(sh);

            Header h = sipProvider.getHeaderFactory().createHeader("P-Access-Network-Info",
        ...
        try
        {
            ...
            tiempoInicioRegistro = Calendar.getInstance().getTimeInMillis();
            String time = Long.toString(tiempoInicioRegistro);
            logger.info("Tiempo de inicio de Registro: " + time);
            logger.debug("sent request= " + request.getMethod());
    ...
    public long getTiempoInicioRegistro(){
        return tiempoInicioRegistro;

    }


    public long getTiempoInicioDesregistro(){
        return tiempoInicioDesRegistro;
    }
                          ...
                          Response          response)
    {
        ...
        //Si se trata de una desconexion, no actualizamos la cabecera RouteHeader
        ExpiresHeader exp = (ExpiresHeader) clientTransatcion.getRequest().getHeader(ExpiresHeader.NAME);
        if (exp.getExpires() != 0 )
            processServiceRoute(response);
     ...
     * @param response
     */
    private void processServiceRoute(Response response){
        ServiceRoute sr = ( (ServiceRoute) response.getHeader(ServiceRoute.NAME));
        if (sr != null) {
            try {
                //<sip:orig@scscf.open-ims.test:6060;lr>
                CSeqHeader cseq = (CSeqHeader) response.getHeader(CSeqHeader.NAME);
                AddressImpl addr = (AddressImpl) sipProvider.getAddressFactory().createAddress(sr.getHeaderValue());
                sipProvider.setRoute(cseq.getMethod(), new Route(addr));
            } catch (ParseException ex) {
                logger.error("Error while trying to generate route header");
            }
        }
    }
    ...
    public void unregister() throws OperationFailedException
    {
        ...
        try
        {
            ...
            unregisterRequest.removeHeader(ExpiresHeader.NAME);
            ...
            if(via != null) {
                ...
                via.setBranch(ProtocolProviderServiceSipImpl.BRANCH_BEGIN +
                        sipProvider.getRandomHex(16) +
                        ProtocolProviderServiceSipImpl.BRANCH_END);
            }
        }
        catch (ParseException ex) {
            logger.error("Error while generating branch-Id", ex);
        }
        catch (InvalidArgumentException ex)
        {
        ...
        try
        {
            ...
            tiempoInicioDesRegistro = Calendar.getInstance().getTimeInMillis();
            String time = Long.toString(tiempoInicioDesRegistro);
            logger.info("Tiempo de inicio del Des-registro: " + time);
            logger.debug("sent request: " + unregisterRequest.getMethod());
    ...
    /**
     *
     * @return the String of our registrar domain
     */
