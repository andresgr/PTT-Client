...
import es.umu.mvia.imsclient.util.xml.*;
    ...
    implements OperationSetPersistentPresence, SipListener
{
    ...
    private SIPIfMatchHeader ifmHeader;
    ...
    private static String tupleid = null; 
    private static String personID = null;
    private static String presenceState = null;


    // XML documents types
    private static final String PIDF_XML        = "pidf+xml";
    private static final String RMLI_XML        = "rlmi+xml";
    ...
    private static final String NOTE_ELEMENT    = "dm:note";
    
    private static final String PRIORITY_ATTRIBUTE  = "priority";

    // rpid elements and attributes
    private static final String CIPID_NS_ELEMENT = "xmlns:c";
    private static final String CIPID_NS_VALUE = "urn:ietf:params:xml:ns:pidf:cipid";
    ...
    //RMLI constants
     private static final String NS_RMLI_VALUE = "urn:ietf:params:xml:ns:rlmi";
            ...
            int subscriptionExpiration)
    {
        ...
        //this.parentProvider.registerEvent("message-summary");
               ...
               OperationFailedException
    {
        ...
        if (this.useDistantPA) {
            ...
            //Adding IMS Headers
            if (parentProvider.hasRouteHeader(req.getMethod()))
                req.setHeader(parentProvider.getRouteHeader(req.getMethod()));

            req.addHeader(parentProvider.getAllowHeader());
            req.addHeader(parentProvider.getIMS_OSGi_ClientAgentHeader());
     ...
     */
    private CallIdHeader publishCallIdHeader = null;
        ...
        throws OperationFailedException
    {
        ...
        try
        {
            destinationInetAddress = parentProvider.getOutboundProxySocketAddress() != null ?
                parentProvider.getOutboundProxySocketAddress().getAddress() :
                InetAddress.getByName(((SipURI) this.parentProvider.getOurSipAddress().getURI()).getHost());
        }
        ...
        // Call ID
        //CallIdHeader
        if (publishCallIdHeader == null) {
            publishCallIdHeader = this.parentProvider
                .getDefaultJainSipProvider().getNewCallId();

            try {
                publishCallIdHeader.setCallId(parentProvider.getRandomString(43).concat("."));
        ...
        try {
            if (this.distantPAET != null && ifmHeader == null) {
        ...
        //Contact
        req.setHeader((Header)parentProvider.getContactHeader());
               ...
               OperationFailedException
    {
        ...
        try
        {
            subscription = createSubscription(contact,this.subscriptionDuration, "presence");
        }
        ...
        //Adding IMS headers
        if (parentProvider.hasRouteHeader(subscription.getMethod()))
            subscription.setHeader(parentProvider.getRouteHeader(subscription.getMethod()));

        subscription.addHeader(parentProvider.getAllowHeader());
        subscription.addHeader(parentProvider.getIMS_OSGi_ClientAgentHeader());
    ...
    throws OperationFailedException {
    	return createSubscription(contact, expires, "presence");
    }
     ...
     * @param contact The contact concerned by this subscription
     * @param expires The expires value
     * @param eventType The presence type to send
     *
     * @return a valid sip request representing this message.
     *
     * @throws OperationFailedException if the message can't be generated
        ...
        throws OperationFailedException
    {
        ...
        try
        {
        	//logger.debug("Mi address: " + contact.getAddress());
            toAddress = parseAddressStr(contact.getAddress());
            if (eventType.equalsIgnoreCase("presence.winfo") && toAddress != null) {
                toAddress.setDisplayName(null);
            }
        ...
        .getDefaultJainSipProvider().getNewCallId();
        try {
            callIdHeader.setCallId(parentProvider.getRandomString(43).concat("."));
		} catch (ParseException e) {
			e.printStackTrace();
		}
        ...
        try {
            evHeader = this.parentProvider.getHeaderFactory()
                .createEventHeader(eventType != null ? eventType : "presence");
            //evHeader = this.parentProvider.getHeaderFactory().createEventHeader("eventlist");
        ...
        // Accept
        if (eventType != null && (eventType.equals("presence.winfo") || eventType.equals("presence")) ) {
	        AcceptHeader accept = null;
	        try {
	            if (eventType != null && eventType.equals("presence.winfo"))
	        			accept = this.parentProvider.getHeaderFactory()
		                .createAcceptHeader("application", WATCHERINFO_XML);
	        	else {
                    accept = this.parentProvider.getHeaderFactory().createAcceptHeader(
                            "multipart/related, application/rlmi+xml, application", PIDF_XML);
                }
        ...
        //Subject
        //if (eventType != null && eventType.equals("presence"))
        //	req.setHeader(new Subject());

        /*QUITAR ESTO*/
        try {
            req.addHeader(parentProvider.getHeaderFactory().createHeader("Supported", "eventlist"));
            req.addHeader(parentProvider.getHeaderFactory().
                    createAcceptHeader("application", "rlmi+xml"));
        ...
        throws OperationFailedException
    {
        ...
        List<RouteHeader> routeList = new ArrayList<RouteHeader>();
        if (req.getHeader(RouteHeader.NAME) != null) {
            Iterator<RouteHeader> it = req.getHeaders(RouteHeader.NAME);
            while (it.hasNext())
                routeList.add(it.next());
            req.removeHeader(RouteHeader.NAME);
        }

        try {
			req.setMethod(Request.SUBSCRIBE);
		} catch (ParseException e) {
        ...
        try
        {
        	SipStack ss = parentProvider.getJainSipStack();
        	SipProvider sp = parentProvider.getDefaultJainSipProvider();
        	
        	try
            {
                ViaHeader viaHeader = parentProvider.getHeaderFactory().
                createViaHeader(
            		parentProvider.getOutboundProxySocketAddress().getAddress().toString(),
                    parentProvider.getOutboundProxySocketAddress().getPort(),
                    parentProvider.getDefaultTransport(),
                    null);
                
                req.setHeader((Header)viaHeader);
            }catch (Exception ex){}

        	transac = this.parentProvider.getDefaultJainSipProvider()
                .getNewClientTransaction(req);

            transac.getDialog().getRemoteParty();
        }
        catch (TransactionUnavailableException ex)
        {
            ex.printStackTrace();
        ...
        //ViaHeaders
        ArrayList viaHeaders = this.parentProvider.getLocalViaHeaders(
                destinationInetAddress,parentProvider.getDefaultListeningPoint());
        req.setHeader((Header) viaHeaders.get(0));

        if( ! routeList.isEmpty() ) {
            Iterator<RouteHeader> it = routeList.iterator();
            while (it.hasNext())
                req.addHeader(it.next());
        }
               ...
               OperationFailedException
   {
    ...
    private ClientTransaction createUnSubscription(ContactSipImpl contact, Dialog dialog) {
    	ClientTransaction trans = null;
    	try {
			Request req = createSubscription(contact, 0);
			req.setHeader(contact.getClientDialog().getCallId());
			
			Request serverResponse = null;
	        try {
	        	serverResponse = dialog.createRequest(Request.SUBSCRIBE);
	        } catch (SipException e) {
	            logger.error("Can't create the SUBSCRIBE message", e);
	            throw new OperationFailedException(
	                    "An unexpected error occurred while"
	        ...
	        trans = parentProvider.getDefaultJainSipProvider()
	        .getNewClientTransaction(req);
	        
	        if (serverResponse.getHeader(RouteHeader.NAME) != null) {
	            Iterator<RouteHeader> it = serverResponse.getHeaders(RouteHeader.NAME);
	            while (it.hasNext())
	            	req.addHeader(it.next());
    ...
    public void processResponse(ResponseEvent responseEvent)
    {
        ...
        if (method.equals(Request.SUBSCRIBE)) {
            ...
            if (response.getStatusCode() == Response.OK
                || response.getStatusCode() == Response.ACCEPTED)
            {
                //logger.trace("#######Hemos recibido un OK DESDE EL CORE........");
        ...
        // PUBLISH
            ...
            if (response.getStatusCode() == Response.OK) {
                ...
                try {
                    // keep a margin
                    timer.schedule(republishTask, (subscriptionDuration - REFRESH_MARGIN) * 1000);
                } catch (IllegalArgumentException e) {
                    logger.error("the expires value seems to be less than a " +
                            "minute, let's assume it", e);
                    timer.schedule(republishTask, PRESENCE_DEFAULT_EXPIRE * 1000);
        ...
        throws NullPointerException
    {
        ...
        logger.debug("contact : " + contact.getAddress() + " resolved");
    }
    ...
    public void processRequest(RequestEvent requestEvent)
    {
        ...
        if (request.getMethod().equals(Request.NOTIFY)) {
            ...
            //logger.debug("notify de: " + request.getHeader(FromHeader.NAME) + "received");
                    ...
                    .isResolved())
            {
                ...
                try {
                    ...
                    logger.debug("Error while sending OK response: " + ex.getMessage());
                }
                // can't finalize the subscription here : the client dialog is
                // null until the reception of a OK
            }
            ...
            /*if (contact == null && !sstateHeader.getState().equalsIgnoreCase(
                ...
                // this end (ifr it's the last notify of a subscription we just
                // stopped
                synchronized (this.waitedCallIds) {
                    this.waitedCallIds.remove(idheader.getCallId());
                }
            ...
            response.addHeader(parentProvider.getContactHeader());
            response.addHeader(parentProvider.getIMS_OSGi_ClientAgentHeader());
                     ...
                     SubscriptionStateHeader.TERMINATED))
            {
                ...
                ContentTypeHeader ctheader = (ContentTypeHeader) request
                .getHeader(ContentTypeHeader.NAME);
                //logger.debug("Content-type del NOTIFY: " + ctheader);

                //Procesamos el contenido multipart
                //if (ctheader.getContentType() == "multipart/related") {
                if ( ((ContentLengthHeader)request.getHeader(ContentLengthHeader.NAME)).getContentLength() > 0 ) {

                    parseMultipartPresenceStatus(new String(request.getRawContent()));
                    /*parseMultipartPresenceStatus(new String(request.getRawContent()),
                            ctheader.getParameter("boundary"));
        ...
        // SUBSCRIBE
        	...
        	logger.debug("Recibido SUBSCRIBE de " + from.getName());
    ...
    */
     ...
     */
     public byte[] getPidfPresenceStatus(ContactSipImpl contact)
     {
         ...
         presence.setAttribute(CIPID_NS_ELEMENT, CIPID_NS_VALUE);
         ...
         if (tupleid == null)
        	 tupleid = "t" + parentProvider.getRandomHex(8);
         ...
         if (personID == null)
        	 personID = "p" + parentProvider.getRandomHex(8);
         person.setAttribute(ID_ATTRIBUTE, String.valueOf(personID));
         ...
         Element onl = doc.createElement(RPID + contact.getPresenceStatus().getStatusName().trim());
         activities.appendChild(onl);
         ...
         String s= contact.getPresenceStatus().getStatusName();
         Element noteNodeEl = doc.createElement(NOTE_ELEMENT);
         if (statusMessage == null)
            noteNodeEl.appendChild(doc.createTextNode(contact.getPresenceStatus()
                 .getStatusName()));
         else
            noteNodeEl.appendChild(doc.createTextNode(statusMessage));
             
         person.appendChild(noteNodeEl);
         presenceState = s;
     ...
     private void showXML(Node n) {
         logger.debug("Nodo: " + n.toString());
...
<?xml version="1.0" encoding="UTF-8" standalone="yes"?><list fullState="false" version="4" uri="sip:andres-contacts@open-ims.test" xmlns="urn:ietf:params:xml:ns:rlmi"><resource uri="sip:asgarcia@open-ims.test"><instance cid="sip:asgarcia@open-ims.test" state="active" id="sip:asgarcia@open-ims.test"/></resource></list>

--c557c2e2
Content-Transfer-Encoding: binary
Content-ID: <sip:asgarcia@open-ims.test>
Content-Type: application/pidf+xml;charset="UTF-8"

<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<presence entity="sip:asgarcia@open-ims.test" xmlns:ns2="urn:oma:xml:prs:pidf:oma-pres" xmlns="urn:ietf:params:xml:ns:pidf" xmlns:ns4="urn:ietf:params:xml:ns:pidf:data-model" xmlns:ns3="urn:ietf:params:xml:ns:pidf:rpid" xmlns:ns9="urn:oma:xml:prs:pres-rules" xmlns:ns5="urn:ietf:params:xml:ns:common-policy" xmlns:ns6="urn:ietf:params:xml:ns:pres-rules" xmlns:ns7="urn:ietf:params:xml:ns:xcap-diff" xmlns:ns8="urn:ietf:params:xml:ns:resource-lists">
<tuple id="td6aa372e">
<status>
<basic>open</basic>
</status>
</tuple>
<ns4:person id="p40b99172">
<ns3:activities>
<rpid:Online xmlns:rpid="urn:ietf:params:xml:ns:pidf:rpid"/>
</ns3:activities><ns4:note>Online</ns4:note>
</ns4:person>
</presence>
...
--c557c2e2--
     ...
     public void parseMultipartPresenceStatus(String presenceDoc) {

         List<String> contacts = null;
         String boundary = presenceDoc.substring(0, presenceDoc.indexOf('\n'));
         //logger.info("Parseando: " + presenceDoc);

         String aux = presenceDoc.substring(boundary.length() + 1);
         String part = aux.substring(0, aux.indexOf(boundary));
         if (part.contains(CONTENT_ID) && part.contains(CONTENT_TYPE)) {
            if (part.contains(RMLI_XML)) {
                contacts = processRmliPart(part.substring(part.indexOf("<?")));
            }
         }
         aux = aux.substring(aux.indexOf(boundary) + boundary.length() + 1);

         //Añadir contactos a la lista local
         Iterator<String> it = contacts.iterator();
         while (it.hasNext()) {
            String c = (String)it.next();
            if (resolveContactID(c) == null)
                ((ContactGroupSipImpl) this.contactListRoot).addContact(
                        new ContactSipImpl(c,this.parentProvider));
         }

         while (aux.indexOf(boundary) != -1) {
            part = aux.substring(0, aux.indexOf(boundary));
            if (part.contains(CONTENT_ID) && part.contains(CONTENT_TYPE) && part.contains(PIDF_XML)) {
                    String user = part.substring(part.indexOf(CONTENT_ID) + CONTENT_ID.length() + 1,
                        part.indexOf(CONTENT_TYPE) - 2);
                    if(contacts.contains(user)) {
                       //setPidfPresenceStatus(part.substring(part.indexOf("<?")));
                       ContactSipImpl contact = (ContactSipImpl)resolveContactID(user);
                       SipStatusEnum status = new SipStatusEnum(part.substring(part.indexOf("<?")));
     ...
     private List<String> processRmliPart(String presenceDoc) {
         ...
         NodeList contactslist = doc.getElementsByTagNameNS(NS_RMLI_VALUE,LIST);

         if (contactslist.getLength() == 0) {
             logger.error("no list element in this document");
             return contacts;
         }

         if (contactslist.getLength() > 1) {
             logger.info("more than one list element in this document");
         }

         Node presNode = contactslist.item(0);
         if (presNode.getNodeType() != Node.ELEMENT_NODE) {
             logger.error("the presence node is not an element");
             return contacts;
         }
         Element listElement = (Element) presNode;
         if (listElement.hasAttribute(URI)) {
             String listUri = listElement.getAttribute(URI);

             /*if (contactListRoot.findContactByID(listUri) == null) {
                System.out.println("URI de la lista: " + listUri);
                return contacts;
             }*/
         }

         NodeList resourceList = listElement.getElementsByTagName(RESOURCE);
         for (int i=0; i < resourceList.getLength(); i++) {
             Node resourceNode = resourceList.item(i);
             if (resourceNode.getNodeType() == Node.ELEMENT_NODE) {
                 Element e = (Element)resourceNode;
                 String s = e.getAttribute(URI);
                 contacts.add(s);
             }
         }
         return contacts;
                     ...
                     for (int j = 0; j < statusList.getLength(); j++)
                     {
                         ...
                         if (statusNode.getNodeType() == Node.ELEMENT_NODE)
                         {
                             ...
                             if (statusname.equals(AWAY_ELEMENT)) {
                                 ...
                                 NodeList noteList = person.getElementsByTagName(NOTE_ELEMENT);
                                 if (noteList.getLength() > 0) {
                                    Node noteNode = noteList.item(0);
                       
                                    if (noteNode.getNodeType() == Node.ELEMENT_NODE) {
                                        Element note = (Element) noteNode;
                                        String state = getTextContent(note);
                                        personStatus = new SipStatusEnum(statusname,state);
                                        break;
                                     }
                                 }
                                 personStatus = new SipStatusEnum(statusname);
                             }
                         }
                     }
     ...
     /**
      * Forces the poll of ourselves
      *
      */
     public void forcePollMyself(String eventType) {
         if (this.presenceEnabled == false) {
             return;
         }

         ContactSipImpl c = new ContactSipImpl(parentProvider.getOurSipAddress().toString(),
        		 parentProvider);
         ...
         try
         {
             subscription = createSubscription(c,this.subscriptionDuration,eventType);
        	 /*if (eventType.equalsIgnoreCase("message-summary"))
        		 subscription = createSubscription(c,300,eventType);
        	 else
        		 subscription = createSubscription(c,this.subscriptionDuration,eventType);*/
         }
         catch (OperationFailedException ex)
         ...
         while (groupsIter.hasNext()) {
             ...
             while (contactsIter.hasNext()) {
                 ...
                 if (contact.getClientDialog() != null && contact.isResolved())
                 {
                     ...
                     logger.debug("unsubscribed to " + contact.getAddress());
                 } else {
                     logger.debug("contact " + contact.getAddress()
    ...
    public PresenceStatus containsState(String newState) {
        return SipStatusEnum.getStatus(newState);
    }

     public class RefreshSubscriptionTask extends TimerTask
     {
         /**
     ...
     protected class RePublishTask extends TimerTask
     {
          ...
          */
         public void run() {
             logger.debug("Publicacion periodica del estado");
             Request req = null;
             try {
                 if (distantPAET != null) {
          ...
          */
          public void registrationStateChanged(RegistrationStateChangeEvent evt)
          {
                      ...
                      RegistrationState.REGISTERED))
              {
                   ...
                   //Enviar SUBSCRIBE al RLS
                   /*String userId = parentProvider.getAccountID().getUserID();
                   userId = userId.substring(0, userId.indexOf('@')) +
                               "-contacts" + userId.substring(userId.indexOf('@'));
                   logger.info("Subscribiendo a " + userId);
                    try {
                        subscribe(userId);
                        //("presence");
                    } catch (Exception ex) {
                   ...
                   //Nos subscribimos a nosotros mismos
                   //forcePollMyself("message-summary");
                   //forcePollMyself("presence.winfo");
                   ...
                   while (groupsIter.hasNext()) {
                    ...
                    try {
                        publishPresenceStatus(SipStatusEnum.ONLINE, SipStatusEnum.ONLINE.getStatusName());
                    } catch (IllegalArgumentException e) {
                        logger.error("Unable to set initial presence state." +
                              " Please check method arguments");
                  ...
                  // send our presence status periodically
                    /*republishTask = new RePublishTask();
                    try {
                        timer.schedule(republishTask,0);
                    } catch (IllegalArgumentException e) {
                        logger.error("Not able to send the first PUBLISH message", e);
                    }*/
               }
          }
     }
